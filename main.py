...
from modules.speedbooster import SpeedBooster
...
class Game:
	def __init__(self, level, chef1_points = 0, chef2_points =0):
		...
		self.running = True
		if level == 0:
			self.background = pg.image.load(f"{IMAGE_PATH}startscreen.png")
			...
			self.skip = pg.image.load(f"{IMAGE_PATH}SKIP.png")
			self.bonus_image = pg.image.load(f"{IMAGE_PATH}BONUS.png")
		elif level == 6:
			if chef1_points > chef2_points:
				self.background = pg.image.load(f"{IMAGE_PATH}player1_end.png")
			elif chef2_points > chef1_points: 
				self.background = pg.image.load(f"{IMAGE_PATH}player2_end.png")
			else:
				self.background = pg.image.load(f"{IMAGE_PATH}tie.PNG")
		elif level ==7:
			self.background = pg.image.load(f"{IMAGE_PATH}credit.png")
		else:
			g = Game(0)
			g.run()
			self.running = False
		...
		self.transition_image = pg.image.load(f"{IMAGE_PATH}transition.png")
	...
	def run(self):
		music = [f"{SOUND_PATH}sountrack.wav", f"{SOUND_PATH}song.wav",f"{SOUND_PATH}song2.wav"]
		...
		if self.level == 0:
			...
			if self.level == 0:
				...
				if 700 <  mouse[0] < 800 and 600 < mouse[1] < 850:
					self.background = pg.image.load(f"{IMAGE_PATH}startscreen-select-play.png")
					if click[0] == 1:
						g = Game(1)
						g.run()	
						self.playing = False	
				elif 212 < mouse[0] < 314 and 862 < mouse[1] < 964:
					self.background = pg.image.load(f"{IMAGE_PATH}startscreen-select-tut.png")
					if click[0] == 1:
						t = tut.Tutorial()
						t.run()
						self.playing = False
				elif 42 < mouse[0] < 144 and 862 < mouse[1] < 964:
					self.background = pg.image.load(f"{IMAGE_PATH}startscreen-select-creds.png")
					if click[0] == 1:
						g = Game(7)
						g.run()	
						self.playing = False
			...
			if self.level ==8:
				...
				if 740 < mouse[0] < 950 and 910 < mouse[1] < 960:
					self.skip = pg.image.load(f"{IMAGE_PATH}SKIP-select.png")
					if click[0] == 1:
						g = Game(1)
						g.run()
						self.playing = False
			...
			if 740 <  mouse[0] < 950 and 910 < mouse[1] < 960:
				if self.chef1_points > self.chef2_points:
					self.background = pg.image.load(f"{IMAGE_PATH}player1_end-select-back.png")
				else:
					self.background = pg.image.load(f"{IMAGE_PATH}player2_end-select-back.png")
				if click[0] == 1:
					g = Game(7)
					g.run()	
					self.playing = False
			...
			click = pg.mouse.get_pressed()
			if 740 <  mouse[0] < 950 and 910 < mouse[1] < 960:
				self.background = pg.image.load(f"{IMAGE_PATH}credit-select-back.png")
				if click[0] == 1:
					g = Game(0)
					g.run()
					self.playing = False
	...
	def transition(self):
		...
		click = pg.mouse.get_pressed()
		if self.darkmode:
			self.transition_image = pg.image.load(f"{IMAGE_PATH}darktransition.png")
			...
			if 750 <  mouse[0] < 965 and 925 < mouse[1] < 975:
				self.transition_image = pg.image.load(f"{IMAGE_PATH}transition-next-select.png")
				if click[0] == 1:
					if self.level == 4:
						self.darkmode = True
					else:
